...
While learning Chinese, one is hit with a torrent of vocabulary. This is particularly true when one first tries to read a book. Perhaps four thousand characters and ten thousand vocabulary items must be learned, at least passively, before reading can proceed effectively without a dictionary. One strategy for learning these items would be to add every new item encountered to an [SRS](https://en.wikipedia.org/wiki/Spaced_repetition) deck. However, this results in not only a massive bloat of due cards, but also in a low quality of recall: after just one exposure to a word in-context, the chances of recall are very low --- particularly if the word was not understood from context.
...
*Originally published on [Quasiphysics](https://quasiphysics.wordpress.com/2012/02/16/deuxieme-vue/).*
